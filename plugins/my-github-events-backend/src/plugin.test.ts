...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [myGithubEventsPlugin],
    });

    await request(server).get('/api/my-github-events/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/my-github-events/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/my-github-events/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/my-github-events/todos/${createdTodoItem.id}`)
